...
public class ProyectoInventarioFinal {
...
los arreglos de los productos que se compran y venden */
        ...
        //Metodo para calcular inventario final
        String[] InventarioFinal = DetInventarioFinal(Auxiliar,DescripcionProducto,CantidadProducto,CodProductos,DescripcionCod);
            System.out.printf("El inventario final es: %nDescripcion producto  ||  Cantidad en stock%n"+Arrays.toString(InventarioFinal));
    }

    private static String[] DetInventarioFinal(String[] auxiliar, String[] descripcionProducto, int[] cantidadProducto, String[] codProductos, String[] descripcionCod) {
        String[] CodificacionProductos = new String[descripcionProducto.length];//Antes de calcular el Inventario Final hay que codificar cada uno de los articulos vendidos y comprados que estan en lel arreglo DescripcionProducto
        String Producto="";
        for (int i = 0; i < descripcionProducto.length; i++) {
            Producto=descripcionProducto[i];
                for (int j = 0; j < descripcionCod.length; j++) {
                    if (Producto.equals(descripcionCod[j])){
                        CodificacionProductos[i]=codProductos[j];
                    }
            }
        }
        int []SumaVentas = new int[codProductos.length];
        int []SumaCompra = new int[codProductos.length];
        String Productoitem = "";
        for (int n = 0; n < codProductos.length; n++) {
            Productoitem =codProductos[n];
            for (int k = 0; k < CodificacionProductos.length; k++) {
                if (Productoitem.equals(CodificacionProductos[k]) && auxiliar[k].substring(0,1).equals("V")){
                    SumaVentas[n]+=cantidadProducto[k];
                } else if (Productoitem.equals(CodificacionProductos[k]) && auxiliar[k].substring(0,1).equals("C")){
                    SumaCompra[n]+=cantidadProducto[k];
                }
            }
        }
        int []CalculoInventarioFinal = new int[codProductos.length];
        String[] InventarioFinal =new String[codProductos.length];
        for (int x = 0; x < codProductos.length; x++) {
            CalculoInventarioFinal[x]= SumaCompra[x]-SumaVentas[x];
            InventarioFinal[x]=String.format(descripcionCod[x] + "  ||  " +CalculoInventarioFinal[x]+"%n") ;
        }
